...
def build_cdb_commands(address: int, size: int) -> str:
    # Keep the hardware breakpoint command free of nested .printf quoting. The
    # cdb-agent watch lane already showed that fragile quote nesting can produce
    # a false "no writes" capture before the breakpoint is actually armed.
    return "\n".join(
        [
            ".effmach x86",
            f".printf \"CDB_KEY_WATCH_ARMED address=0x{address:08x} size={size}\\n\"",
            f'ba w{size} {address:08x} "r; ln @eip; u @eip L8; db {address:08x} L{size}; db @ebp L80; db @edi L90; kb 12; qd"',
            "g",
            "",
        ]
    )
...
def start_capture(args: argparse.Namespace) -> dict[str, Any]:
    ...
    capture_id = args.capture_id or f"secret-room-key-counter-watch-{args.pid}-{utc_stamp()}"
...
def inspect_capture(args: argparse.Namespace) -> dict[str, Any]:
    ...
    armed = "CDB_KEY_WATCH_ARMED" in log_text
    hit = ("Breakpoint 0 hit" in log_text) or ("ba w" in log_text and "eax=" in log_text and not still_running)
    claim_kind = "observed-write" if hit else "watch-armed-waiting" if still_running and armed else "watch-ended-no-hit"
    if malformed:
    ...
    summary = {
        ...
        "claim": {
            ...
            "text": (
                "CDB hardware write breakpoint hit; inspect cdb.log for registers, disassembly, memory, and stack."
                if hit
                else "CDB watch is armed and still waiting for the key counter write."
                if still_running and armed
                else "CDB watch ended without an observed hit."
            ),
        },
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Arm or inspect a direct CDB write watch on NbLittleKeys.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    start = subparsers.add_parser("start")
    start.add_argument("--pid", type=int, required=True)
    start.add_argument("--address", type=parse_address, default=NB_LITTLE_KEYS)
    start.add_argument("--size", type=int, choices=(1, 2, 4, 8), default=1)
