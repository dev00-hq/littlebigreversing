...
test "viewer render path draws the bounded Sendell dialog overlay in the sidebar" {
    ...
    try std.testing.expect(hasTraceText(trace, "NAV / DIAL NORMAL B0"));
...
test "viewer render path draws the bounded 19/19 reward overlay in the sidebar" {
    ...
    try std.testing.expect(hasTraceText(trace, "NAV / REWARD NORMAL B0"));
}
...
test "viewer render path draws moving little-key landing target and path" {
    ...
    try std.testing.expect(hasTraceRectColor(trace, .fill_rect, render.rewardCollectibleMotionPathColorForTesting(.little_key)));
}
