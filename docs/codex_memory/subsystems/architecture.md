...
- The original-runtime split is stable: Tavern uses FRA, Scene11 uses debugger snapshots, room-transition seam proof mixes Frida plus `cdb` only when watcher evidence is insufficient, and the committed debug-control lane uses `debug_compass.py`, `heading_inject.py`, and `waypoint_step_probe.py`.
