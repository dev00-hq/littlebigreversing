...
- `port/` stays canonical; `world_geometry.zig` owns geometry, `room_state.zig` the guarded `19/19` room/hero-start seam, `session.zig` world-position seeds, `world_query.zig` containing-zone/target/raw-start/topology diagnostics, `locomotion.zig` step/result packaging, `viewer_shell.zig` fixture seeding plus raw-start/zone/topology/footing diagnostics and schematic/attempt payloads, and `render.zig` display-only cues.
...
- The `19/19` admitted move-option contract is intentional: HUD uses direction/cell/status lines, stderr keeps `direction:cell:status`, and the schematic uses current-cell plus colored `N/E/S/W` target cues. Keep all three runtime-owned.
- The raw-invalid-start contract is intentional: HUD uses `DIAG ...` / `BOUNDS ...` / `NEAR ...`; stderr uses `diagnostic_status=...`, `occupied_coverage=...`, `occupied_bounds=...`, `occupied_bounds_dx=...`, `occupied_bounds_dz=...`, and `nearest_*=...`. Keep that driven by the runtime hero-start probe.
- `19/19` admitted footing is intentional: HUD uses `SURF ...`, stderr uses `current_footing=...`, and both come from `local_topology.origin_surface` plus `origin_standability`. Keep it admitted-path-only.
