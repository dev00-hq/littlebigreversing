...
- The canonical Scene11 debugger lane now uses the bespoke `cdb-agent` CLI under the hood. The integration point is `life_trace_debugger.py`, which now shells out to `cdb-agent --json memory ...` while preserving the existing `Scene11SnapshotReader` contract and `trace_life.py --mode scene11-pair --launch --cdb-path ...` surface.
- Offline validation is green on both sides of that boundary: `py -3 -m unittest tools.test_life_trace` now passes with `50` tests in `littlebigreversing`, `py -3 -m unittest discover -s tests -p "test_*.py"` passes in `D:\repos\cdb-agent`, and `py -3 .\scripts\validate_littlebigreversing.py --phase static` in `D:\repos\cdb-agent` reports `repo_changed: false`. The next meaningful check is a real live `scene11-pair` run through the new adapter.
