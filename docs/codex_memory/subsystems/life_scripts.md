...
- Guarded `19/19` object `2` now has a stateful runtime-backed later reward loop with bounded magic bonus emission plus viewer/main reward diagnostics.
...
- Do not treat the `19/19` reward overlay or `event=bonus_spawn` output as pickup or save/load parity. They are bounded diagnostics over the reward loop.
...
- When guarded `19/19` object `2` should move beyond bounded reward diagnostics into live `cdb-agent` parity work for motion, pickup, and save/load behavior.
