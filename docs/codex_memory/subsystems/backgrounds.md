...
- Port `187/187` zone `1` no-readjust cube `185` rejects decoded landing `(13824,5120,14848)` as a height mismatch. The previous `(28416,2304,21760)` live landing proof is invalidated because the source teleport produced `zones=[]`, `new_cube=-1`, and a visible clover/life-loss indicator; do not treat it as a zone landing.
