...
- Port `187/187` zone `1` no-readjust cube `185` rejects decoded landing `(13824,5120,14848)` as a height mismatch. `tools/fixtures/phase5_187_runtime_proof.json` proves live instead lands at `(28416,2304,21760)` from saved context; `tools/fixtures/phase5_187_startcube_counterfactual.json` shows changing `StartCube=(55,11,44)` to `(54,11,44)` alone does not move that landing.
