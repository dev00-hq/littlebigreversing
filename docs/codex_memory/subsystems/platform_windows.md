...
- Original `LBA2.EXE` needs external DxWnd at `C:\Users\sebam\DxWnd.reloaded\build\dxwnd.exe`. Bad custom profiles can hang in `DDRAW`/`dxwnd`/`apphelp`; the built-in template is more startup-stable, but `winver0=1` can crash at `0x0045BFAE` (`MOV EAX, CR0` inside `FUN_0045BD62`).
- Best-known profile: `tools/life_trace/profiles/lba2_built_in_flag_core_candidate.dxw` (built-in base, `winver0=0`, stable flag core, windowed presentation cluster). A room-36 direct-save smoke on `2026-04-20` survived 30 seconds windowed with no `Application Error`.
- That profile is startup-stable, not long-run-proven. On `2026-04-28`, it still hung after startup with exact candidate fields loaded; evidence moved to DirectDraw release/lock activity. `LBA2+0x5bae8` / `0x5bb69` are DirectDraw `Lock` / `Unlock` wrappers, so do not blame Miles solely from nearby `mss32!AIL_service_stream` stack context.
