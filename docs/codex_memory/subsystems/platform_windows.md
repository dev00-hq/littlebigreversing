...
- In the current full-asset Windows workspace, `zig build test-fast` can still be too slow to use as an always-on acceptance loop. Keep targeted `zig build tool -- ...` probes plus `scripts/verify_viewer.py` available for iteration when the full shard is not practical.
