...
- Keep `runtime/session.zig` on explicit world-position seeds, not `RoomSnapshot` init.
- Keep `runtime/world_query.zig` as the guarded pure query owner, including exact containing-zone membership and raw target-cell evaluation.
- Keep `runtime/locomotion.zig` as the guarded `19/19` step/result seam while future widening stays diagnostics-only.
- Keep `app/viewer_shell.zig` as the viewer-local locomotion-summary owner: HUD uses direction/cell/status lines plus `ZONES ...`, stderr uses structured move-option and `zones=...` fields, and both derive from runtime-owned data.
...
- Guarded `19/19` viewer/runtime diagnostics: keep runtime-owned containing-zone membership and admitted-path target-cell evidence explicit in HUD and stderr on seeded, accepted, and admitted-position rejected statuses only.
...
- Keep the next guarded `SCENE.HQR[19]` slice on movement-semantic cues beyond landed zone-summary and target-cell diagnostics, not on new movement policy or invented zone heuristics.
