...
- Align implementation planning around the Zig 0.15.2 + SDL2 port direction and use the next bounded Phase 2 step to inventory scene-local life-program evidence before any typed decoder lands there.
...
- Phase 2 typed asset decoding now preserves raw hero/object track and life blobs, and track programs are decoded structurally, but life programs are still opaque byte arrays with no source-backed typed surface yet.
...
- Audit `LM_*` opcode evidence before attempting any scene-local life-program decoder or gameplay interpretation.
