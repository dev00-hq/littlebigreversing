...
- Keep the runtime/load seam fail-fast on unsupported scene life; `LM_DEFAULT` and `LM_END_SWITCH` stay out unless new checked-in primary-source evidence appears.
...
- Keep mapping falsification-oriented: only `hero_start_world_point` is admitted, and `zone_world_point` still outranks `scene_object_world_point` on bounded `19/19`.
...
- Canonical `zig build test` and `zig build run` still depend on the extracted asset tree and Windows SDL2 layout.
