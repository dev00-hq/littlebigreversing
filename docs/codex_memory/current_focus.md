...
- Keep `app/viewer_shell.zig` owning HUD/stderr copy for `ZONES`, `TOPO`, `SURF`, widened `move_options=...`, rejected-target coverage, and raw-start `DIAG` / `BOUNDS` / `NEAR`.
...
- Guarded negative-load diagnostics: keep `ViewerUnsupportedSceneLife` fail-fast, but surface the first blocking opcode/id/offset for `2/2`, `44/2`, and `11/10` on `inspect-room`.
...
- Keep the next guarded `19/19` slice beyond landed zone, target-cell, move-option coverage, rejected-target coverage, topology/current-footing, raw-start coverage/nearest-candidate, and schematic/attempt cues.
