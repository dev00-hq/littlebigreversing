...
- Continue the decode-first port path beyond the height-aware interior composition debug view, the canonical `2/2` zero-fragment probe, the landed synthetic brick-index overlays, and the new actual `BRK`-backed swatch previews.
...
- Scene and background inspection plus the height-aware interior composition debug viewer, zero-fragment `2/2` probe, fragment-bearing `11/10` GRM evidence path, synthetic brick-index overlays, and actual `BRK`-backed swatch previews as viewer-prep surfaces.
...
- Keep the next viewer slice scoped to the checked-in fragment-bearing `11/10` interior pair and richer viewer-local use of the landed `BRK` swatch evidence, not gameplay, life binding, exterior loading, or a full room-art renderer.
