...
- Continue the decode-first port path beyond the height-aware interior composition debug view, the canonical `2/2` zero-fragment probe, the landed synthetic brick-index overlays, the actual `BRK`-backed swatch previews, and the refined `11/10` fragment comparison surface that now prioritizes any composition-vs-fragment delta over exact matches and supports deterministic ranked/cell navigation.
...
- Scene and background inspection plus the height-aware interior composition debug viewer, zero-fragment `2/2` probe, fragment-bearing `11/10` GRM evidence path, synthetic brick-index overlays, actual `BRK`-backed swatch previews, and the refined viewer-local fragment comparison panel / focus highlight / selected-cell detail strip / deterministic navigation as viewer-prep surfaces.
...
- Build future viewer-local refinement on the landed `11/10` fragment comparison prioritization, selected-cell detail strip, and deterministic navigation rather than replacing the viewer-local snapshot path, and keep it away from gameplay, life binding, exterior loading, or a full room-art renderer.
