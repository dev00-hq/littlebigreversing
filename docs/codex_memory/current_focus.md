...
- Continue the decode-first port path beyond the height-aware interior composition debug view, the canonical `2/2` zero-fragment probe, the landed synthetic brick-index overlays, the actual `BRK`-backed swatch previews, and the new deterministic fragment comparison panel for the checked-in `11/10` pair.
...
- Build future viewer-local refinement on the landed `11/10` fragment comparison panel and focus highlight rather than replacing the viewer-local snapshot path, and keep it away from gameplay, life binding, exterior loading, or a full room-art renderer.
