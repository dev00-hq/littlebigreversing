...
Generated by:
  python tools/codex_memory.py briefing --task 'behavior animation root motion comparison'

Source hash:
  sha256:1ce1c1fe848693c4f448cbd194b1d71d55b7124ec7a8f3f323a67e47be57e02e
-->

## Task

behavior animation root motion comparison
...
Selected because: id token match `animation`, `behavior`; tag token match `animation`, `behavior`; body token match `animation`, `behavior`, `motion`, `root`
...
Selected because: body token match `animation`
...
- Behavior-mode movement animation ownership is narrowed but still unresolved. The Otringal animation-correlation probe sampled candidate hero object fields at `0x0049A198`; position candidates tracked the hero, but `gen_body`, `gen_anim`, `next_gen_anim`, sprite candidate, and flag candidate stayed constant while movement began and continued. The adjacent `D:\repos\reverse\lba2-lm2-viewer` decoder now gives stronger positive correlation: `tools/behavior_animation_root_motion_compare.py` maps File3D objects `0..3` to Normal/Sporty/Aggressive/Discreet walk-family ANIM entries and compares decoded `2.00s` root Z against live Otringal movement within about `1.64%` max error. This supports an animation-root-motion implementation hypothesis, but it is not yet proof that the original runtime commits ANIM root deltas directly. Remaining work: find the live frame/phase field or writer, or deliberately adopt animation-derived movement as a behavior-faithful port model with this limitation called out.
  Selected because: token match `animation`, `behavior`, `motion`, `root`
