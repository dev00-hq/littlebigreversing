# Next Step: Viewer-Local Refinement of the Landed 11/10 Fragment Comparison Panel
...
Life integration remains blocked on `LM_DEFAULT` and `LM_END_SWITCH`, so the next bounded slice should stay on the viewer-prep path. Keep `SCENE.HQR[2]` plus `LBA_BKG.HQR[2]` as the canonical zero-fragment acceptance boundary, keep `SCENE.HQR[11]` plus `LBA_BKG.HQR[10]` as the checked-in fragment-bearing interior evidence pair, and use that positive pair to drive the next viewer-local refinement. The intended frontier is no longer first delivery of actual `BRK` swatches, the first fragment comparison panel / focus highlight, or the first selected-cell detail strip, because those surfaces are already landed. The next step is to refine the landed comparison-panel, focus-highlight, and selected-cell-detail surface for the checked-in fragment pair: clearer deterministic composition-vs-fragment delta surfacing, tighter comparison-panel prioritization, richer inspection of the already-landed detail strip, or another equally narrow viewer-local refinement that makes the `11/10` evidence easier to inspect without widening into a room renderer.

Treat background fragment library facts and projected runtime fragment zones as separate surfaces. `port/src/game_data/background.zig` remains the canonical owner of decoded background linkage, composition, fragment-library facts, and the decoded `BRK` preview-library data consumed by the viewer. The `port/src/app/viewer/` modules remain the runtime edge that can project scene `grm` zones, compose viewer-local render state, and turn the checked-in evidence into debug-visible cues. Keep `port/src/tools/cli.zig` as the probe surface, not as the runtime implementation path. If checked-in raster or brick evidence is incomplete, stop and deepen evidence instead of inventing a fallback parser, compatibility path, guessed same-index pairing, or alternate runtime layer.
...
- Add only the minimum viewer-local refinement needed on top of the landed comparison panel, focus highlight, and selected-cell detail strip for the checked-in fragment pair.
  - Treat the existing `drawBrickProbe` output as a synthetic probe surface, and the fixed-size actual swatches plus deterministic fragment comparison panel / focus highlight / selected-cell detail strip as the completed evidence surfaces.
  - Prefer clearer deterministic delta surfacing, tighter comparison-panel prioritization, or deeper use of the existing selected-cell detail strip over another new callout surface or more generic sparse sampling.
...
- Keep decoding canonical and presentation viewer-local.
  - Keep `game_data/background.zig` responsible for canonical background decoding and the decoded `BRK` preview-library data, not SDL-facing presentation policy.
  - Keep scene-bound projection and render-state helpers local to the `port/src/app/viewer/` modules unless a stronger stable seam becomes necessary.
...
- `cd port && zig build run -- --scene-entry 11 --background-entry 10`
  - Verify the window stays open until quit, the viewer still renders the landed height-aware composition path plus the existing synthetic brick probes, actual swatches, and fragment comparison panel / focus highlight / selected-cell detail strip, and the checked-in positive pair remains the runtime acceptance path for the new refinement.
...
- The current viewer brick patterns are still synthetic probes, but fixed-size actual `BRK` swatch previews and a deterministic fragment comparison panel / focus highlight / selected-cell detail strip are already landed beside them.
